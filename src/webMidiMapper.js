...
class WebMidiMapper {
  ...
  nextValue (inputId, event, handler, channel = 'all') {
    ...
    const cancelFn = () => {
      events.forEach(e => {
        inputs.forEach(input => input.removeListener(e, channel, _handler))
      })
    }

    events.forEach(e => {
      inputs.forEach(input => input.addListener(e, channel, _handler))
    })
    return {
      cancel: cancelFn
    }
